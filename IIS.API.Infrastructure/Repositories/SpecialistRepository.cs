...
public sealed class SpecialistRepository : ISpecialistRepository
{
    ...
    {
        return _specialists.FirstOrDefaultAsync(filtres, token);
    }

    public Task<IEnumerable<Specialist>> GetSpecialistsAsync(Expression<Func<Specialist, bool>>? filtres, CancellationToken token)
    {
        if (filtres is null)
            return Task.FromResult(_specialists.AsEnumerable());

        return Task.FromResult(_specialists.Where(filtres).AsEnumerable());
    }
